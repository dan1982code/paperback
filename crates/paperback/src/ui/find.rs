...
fn bind_cancel_button(
	...
	button.on_click(move |_| {
		tracing::debug!("find: Cancel clicked, hiding dialog");
...
fn bind_find_dialog_actions(params: FindDialogActionParams) {
	...
	dialog.on_close(move |event| {
		tracing::debug!("find: window close event, hiding dialog");
...
fn do_find_all(
	...
	let Some(_find_guard) = state.try_begin_find() else {
		tracing::debug!("find all: skip, a find is already in progress");
		return;
	};
	if !ensure_query(state, doc_manager) {
		state.dialog.show(true);
		state.dialog.raise();
		state.focus_find_text();
		tracing::debug!("find all: skip, query is empty");
		return;
	}
	...
	let (rows, origin) = {
		...
		let Some(tab) = dm.active_tab() else {
			tracing::debug!("find all: skip, no active tab");
			return;
		};
		if !tab.text_ctrl.is_valid() {
			tracing::debug!("find all: skip, text control invalid");
			return;
		}
	...
	tracing::debug!(query = %query, rows = rows.len(), ms = started.elapsed().as_millis(), "find all: search + grouping done");
	if rows.is_empty() {
		...
		state.dialog.raise();
		state.focus_find_text();
		tracing::debug!("find all: no matches, stayed in query view");
		return;
	}
	...
	*state.result_rows.borrow_mut() = rows;
	...
	// list before it grows to its full size - populating a list the reader has already focused is
	// far more reliable than focusing a list that just appeared with tens of thousands of rows.
	state.switch_to_results_view();
	state.results_list.set_focus();
	tracing::debug!(ms = started.elapsed().as_millis(), "find all: empty results view shown and focused");
	populate_results_list(state.results_list, &state.result_rows);
	tracing::debug!(rows = row_count, ms = started.elapsed().as_millis(), "find all: results populated");
	...
	state.results_list.set_focus();
	tracing::debug!(selected, ms = started.elapsed().as_millis(), "find all: results list focused, done");
}
...
fn handle_result_go(
	...
	tracing::debug!("find all: Go / list activation triggered");
	...
	drop(dm);
	tracing::debug!("find all: Go jumped to a result, hiding dialog");
...
fn do_find(
	...
	navigation::select_doc_range(tab, start, end);
	drop(dm);
	tracing::debug!("find: a match was found, hiding dialog and selecting it");
	state.dialog.show(false);
	if dialog_was_shown {
